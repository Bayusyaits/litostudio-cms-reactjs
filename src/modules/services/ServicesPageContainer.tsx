...
export default function ServicesPageContainer() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['services', activeSite?.id, filter],
    queryFn: () =>
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['services', activeSite?.id] })
      setSelectedIds([])
    },
  })

  const bulkDeleteMutation = useMutation({
    mutationFn: ({ ids }: { ids: string[] }) => servicesService.bulkDelete({ ids }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['services', activeSite?.id] })
      setSelectedIds([])
    },
  ...
  return (
    <ServicesPageView
      ...
      onOpenEditor={(id) => navigate(`/services/${id}/edit`)}
