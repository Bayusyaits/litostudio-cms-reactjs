...
// BlogPageView.tsx exactly, relabeled for Article.
...
import type { ArticlePost } from '@/types/content.types'
...
export function ArticlePageView({
  ...
  const columns: Column<ArticlePost>[] = [
    {
      key: 'title',
      header: 'Title',
      sortable: true,
  ...
  return (
    <div className="p-6 space-y-5 overflow-y-auto">
      <div className="flex items-center justify-between gap-4">
        <div>
          <h1 className="font-display text-2xl font-bold text-[var(--text-primary)]">Article</h1>
          <p className="font-body text-sm text-[var(--text-muted)] mt-0.5">
            {meta ? `${meta.total} articles` : 'Manage articles'}
          </p>
        </div>
        <Button skin="cms" leftIcon={<Plus className="w-4 h-4" />} onClick={onNew}>New Article</Button>
      </div>
