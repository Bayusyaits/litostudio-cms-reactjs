// 2026-08-10 (user-requested independence split): mirrors modules/journal/
// JournalPageView.tsx exactly, relabeled for Portfolio. Independent
// component — not a wrapper around JournalPageView/StoriesPageView.
import { Plus, LayoutGrid, Trash2, LayoutTemplate, PenLine, Search, X } from 'lucide-react'
        ...
        <div className="flex items-center gap-3">
          {post.cover_image ? (
            ...
            <div className="w-9 h-9 rounded bg-[var(--lito-gold-soft)] flex items-center justify-center flex-shrink-0">
              <LayoutGrid className="w-4 h-4 text-[var(--lito-gold)]" aria-hidden />
            </div>
          )}
  ...
  return (
    <div className="p-6 space-y-5 overflow-y-auto">
      <div className="flex items-center justify-between gap-4">
        <div>
          <h1 className="font-display text-2xl font-bold text-[var(--text-primary)]">Portfolio</h1>
          <p className="font-body text-sm text-[var(--text-muted)] mt-0.5">
            {meta ? `${meta.total} items` : 'Manage portfolio items'}
          </p>
        </div>
        <Button skin="cms" leftIcon={<Plus className="w-4 h-4" />} onClick={onNew}>New Item</Button>
      </div>
      ...
      <div className="cms-card overflow-hidden">
        <DataTable
          ...
          emptyDescription="Add work to build your portfolio"
