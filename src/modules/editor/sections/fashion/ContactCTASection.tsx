...
export function ContactCTASection({ block }: { block: Block }) {
  ...
  const heading = String(d.title ?? d.heading ?? "Get in\nTouch")
  const cta1    = String(d.cta1Label ?? 'Send Message')
  const cta2    = String(d.cta2Label ?? 'View Stores')
  const [h1, h2 = ''] = heading.split('\n')

  return (
    <section style={{ position: 'relative', minHeight: 340, overflow: 'hidden', color: '#fff' }}>
      <AppImage src={image} alt=""
        objectFit="cover"
        wrapperStyle={{ position: 'absolute', inset: 0 }}
        style={{ width: '100%', height: '100%' }} />
      <div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.5)' }} />
      <div style={{ position: 'relative', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center', padding: '72px clamp(24px,2.9vw,40px)', minHeight: 340 }}>
        <h2 style={DISP({ fontSize: 'clamp(40px,5vw,72px)', fontWeight: 800, lineHeight: 0.92, letterSpacing: '-0.025em', textTransform: 'uppercase', margin: '0 0 28px' })}>
          {h1}{h2 && <><br />{h2}</>}
        </h2>
